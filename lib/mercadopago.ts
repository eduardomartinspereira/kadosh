...
if (
  ...
  console.warn(
    "[MP] ⚠️ MERCADOPAGO_ACCESS_TOKEN não definido. Configure no .env"
  );
}
...
export class MercadoPagoService {
  ...
  async createPixPayment(
    ...
    try {
      ...
      throw error?.cause
        ? error
        : new Error("Falha ao processar pagamento PIX");
    }
  }
  ...
  async createCardPayment(args: {
    ...
    try {
      ...
      throw error?.cause
        ? error
        : new Error("Falha ao processar pagamento com cartão");
    }
  }
