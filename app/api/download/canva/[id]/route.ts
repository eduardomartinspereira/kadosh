...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 Debug Canva API - session.user.id:', (session.user as any)?.id)
    ...
    const canvaAsset = product.assets.find(asset => 
      asset.type === 'ZIP' || asset.type === 'OTHER'
    )
    if (!canvaAsset) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      fileName: `${product.name}-canva.${canvaAsset.type === 'ZIP' ? 'zip' : 'zip'}`,
