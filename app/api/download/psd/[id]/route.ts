...
import { checkDownloadPermission, logDownload, checkProductAccess } from '../../../../../lib/download-permissions'
...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 Debug PSD API - session.user.id:', (session.user as any)?.id)
    ...
    if (!hasAccess) {
      return NextResponse.json(
        { error: 'Você não tem acesso a este produto. Verifique sua assinatura.' },
        { status: 403 }
      )
    }
    ...
    if (!permission.canDownload) {
      return NextResponse.json(
        { error: permission.reason || 'Limite de downloads atingido. Verifique sua assinatura.' },
        { status: 429 }
      )
    }
    ...
    if (!(product as any).arquivoPsd) {
      return NextResponse.json(
        { error: 'Arquivo PSD não encontrado para este produto. Entre em contato com o suporte.' },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      downloadUrl: (product as any).arquivoPsd,
    ...
    console.error('🔍 Debug PSD API - Stack trace:', error instanceof Error ? error.stack : 'No stack trace')
